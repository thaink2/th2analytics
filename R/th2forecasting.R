...
#'
#' Sends input data to the Thaink2 Forecasting API and retrieves the forecast
#' results.
#'
...
th2forecast_api <- function(
  ...
  temp_var <- ".index"
  if (temp_var %in% names(forecast_result)) {
    forecast_result <- forecast_result %>%
      dplyr::rename(date_var = !!temp_var) %>%
      dplyr::mutate(temp_var = lubridate::as_datetime(temp_var))
  }
